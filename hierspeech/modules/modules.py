...
from commons.commons import init_weights, get_padding, fused_add_tanh_sigmoid_multiply
...
class FFN_Conv(nn.Module):
    ...
    def __init__(
        ...
        self.fc1 = nn.Conv1d(in_features, hidden_features, kernel_size=kernel, stride=1, padding=(kernel - 1) // 2,
                             bias=bias[0])
        self.act = act_layer()
...
class DiTConVBlock(nn.Module):
    ...
    def __init__(self, hidden_size, num_heads, mlp_ratio=4.0, kernel=9, p_dropout=0.1, **block_kwargs):
        ...
        self.mlp = FFN_Conv(in_features=hidden_size, hidden_features=mlp_hidden_dim, act_layer=approx_gelu,
                            kernel=kernel, p_dropout=p_dropout)
