...
def resample(d_loader, org_sr, target_sr):
    with torch.no_grad():
        for _, audio_path in enumerate(tqdm(d_loader)):         
            y, sr = librosa.load(audio_path, sr=org_sr)
            y_resampled = librosa.resample(y, orig_sr=sr, target_sr=target_sr)
            sf.write(file=audio_path, data=y_resampled, samplerate=target_sr)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--org_sr', default=24000)
